# Requires NomGen and requisites
...
NomGen3Tab <- function(q1,q2,q3, U,N, givetable = FALSE){
  # accepts 3 marginal distributions as vectors
  # as well as U a vector (U(X1,X2),U(X1,X3),U(X2,X3))
  ...
  Tab3 <- NomGenTab(q2, q3, U[3])
  
  A <- get3VarMat2(n1,n2,n3) # matrix describing relations on table entries
  
  b <- c(1, q1[-n1], q2[-n2], q3[-n3], as.numeric(Tab1[-n1,-n2]),as.numeric(Tab2[-n1,-n3]), as.numeric(Tab3[-n2,-n3]))
...
get3VarMat2 <- function(n1,n2,n3){
  # instantiating empty matrix of correct dimensions
  A <- matrix(ncol = n1*n2*n3, nrow = (n1 + n2 + n3 - 2) + (n1-1)*(n2-1) + (n1-1)*(n3-1) + (n2-1)*(n3-1))
  ...
  for(i in 1:((n2-1)*(n3-1))){ # second
    A[n1 + n2 + n3 - 2 + i + (n1-1)*(n2-1) + (n1-1)*(n3-1), ] <- rep(0,n1*n2*n3) # first fill row with zeros
    val <- i
    xi <- 0 # then find which probability row corresponds to
    xj <- 1
    while(val > (n2-1)){
    ...
    xi <- val
    for(l in 1:(n1)){
      A[n1 + n2 + n3 - 2 + i + (n1-1)*(n2-1) + (n1-1)*(n3-1), n1*(xi-1) + (n1*n2)*(xj-1) + l] <- 1
    }
  }
  return(A)
}
